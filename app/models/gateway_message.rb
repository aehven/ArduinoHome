class GatewayMessage < ApplicationRecord
  enum message_type: {
    presentation: 0,
    set: 1,
    req: 2,
  ...
  def self.build(raw_message)
    # https://www.mysensors.org/download/serial_api_20

    parts = raw_message.split(";")

    if parts.length < 5
      raise ArgumentError.new("Invalid message string: #{raw_message}")
    end

    GatewayMessage.new(
      node_id: parts[0].to_i,
      child_sensor_id: parts[1].to_i,
      message_type: parts[2].to_i,
      ack: parts[3].to_i,
      subtype: parts[4].to_i,
      payload: parts[5]
    )
  end
end
